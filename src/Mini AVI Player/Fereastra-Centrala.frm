...
Private Function NormalizeLaunchPath(ByVal value As String) As String
value = Trim$(value)
If Len(value) >= 2 Then
...
Private Function OpenVideoFile(ByVal selectedFile As String) As Boolean
Dim ret As Long
Dim fileTitle As String

On Error GoTo OpenFailed
selectedFile = NormalizeLaunchPath(selectedFile)
If selectedFile = "" Then Exit Function

If Len(Dir$(selectedFile)) = 0 Then
    MsgBox "The selected video file does not exist.", vbCritical, "Mini AVI"
    Exit Function
End If
...
ret = OpenMedia
Debug.Print "[OpenVideoFile] ret after OpenMedia = " & ret
If ret <> 0 Then
    MsgBox "The file cannot be played: " & MciErrorText(ret), vbCritical, "Mini AVI"
    Exit Function
End If
...
fileTitle = FileNameOnly(selectedFile)
TitluFilm.Caption = fileTitle
FisierSubtitrare = fileTitle
Calea_Spre_Lumina = selectedFile
TOT_Subtitrare = ""
...
OpenFailed:
MsgBox "Mini AVI could not open the selected file.", vbCritical, "Mini AVI"
End Function
...
MsgBox "Mini AVI by Paul A. Gagniuc", vbInformation, "About Mini AVI"
End Sub
...
VolumZero = 0
MediaVolume = 80
NivelVolum.Caption = MediaVolume
NivelVolumDisplay.Caption = MediaVolume & " %"
voldisp.Cls
Call BitBlt(voldisp.hDC, 0, 8, 25, 43, volorig.hDC, 0, 8, vbSrcCopy)

Paul = 1
PositionMainWindowBottomRight Me
RegisterAviAssociation

Set CD = New CommonDialog
CD.Filter = "AVI files (*.avi)|*.avi|Known video files|*.avi;*.asf;*.mpg;*.mpeg;*.wmv;*.mp4;*.mkv;*.mov;*.m4v;*.flv;*.divx;*.ts;*.webm|All files (*.*)|*.*"
CD.DialogTitle = "Choose a video file"
...
launchFile = NormalizeLaunchPath(Command$)
If launchFile <> "" Then
    OpenVideoFile launchFile
End If
End Sub
...
Public Sub TogglePlaybackFromMovie()
If Not ApasatButoane Then Exit Sub
