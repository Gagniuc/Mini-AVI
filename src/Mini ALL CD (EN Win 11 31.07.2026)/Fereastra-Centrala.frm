...
CD.ShowOpen
tmp = CD.FileName
Debug.Print "[Deschide_Click] fisier ales = " & tmp
...
MediaPath = """" & tmp & """"
ret = OpenMedia
Debug.Print "[Deschide_Click] ret dupa OpenMedia = " & ret
If ret = 0 Then
...
CD.Filter = "All files (*.*)|*.*|Known video files|*.avi;*.asf;*.mpg;*.mpeg;*.wmv;*.mp4;*.mkv;*.mov;*.m4v;*.flv;*.divx;*.ts;*.webm|DivX File (*.avi)|*.avi"
...
If Command <> "" Then
    ...
    lblSize.Caption = intSize & " %"
    MediaPath = """" & Command & """"
    Debug.Print "[Form_Load] fisier din linia de comanda = " & Command
    ret = OpenMedia
    Debug.Print "[Form_Load] ret dupa OpenMedia = " & ret
    If ret = 0 Then
        Call GetSize
        DurataFilmului.Caption = Format(MediaDuration, "h:mm:ss")
        LungimeAVI = MediaLengthMS
