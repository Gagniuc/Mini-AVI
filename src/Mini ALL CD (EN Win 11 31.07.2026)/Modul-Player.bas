...
Public ApasatButoane As Boolean        'Tells if a media has been choosen
...
'***API used only for diagnostics - translates an MCI error code into readable text***'
...
MciErrorText = Left(buf, InStr(buf, vbNullChar) - 1)
End Function
...
If ret <> 0 Then
    'No display window (ret=346) usually means the media has no video track,
    'e.g. an audio-only file (MP3/WAV) opened through the widened "All files" filter.
    'Nothing to resize/display in that case - fall back to a safe default and continue.
    Debug.Print "[GetSize] no video track detected (probably audio-only) - using default size 320x240"
    MediaWidth = 320
    MediaHeight = 240
    Exit Sub
End If

Debug.Print "[GetSize] raw size string=[" & Trim(size) & "]"
var = Split(size, " ", -1)
Debug.Print "[GetSize] split into " & (UBound(var) - LBound(var) + 1) & " elements"
If UBound(var) >= 3 Then
    Debug.Print "[GetSize] var(2)=[" & var(2) & "]  var(3)=[" & var(3) & "]"
Else
    Debug.Print "[GetSize] not enough elements to read var(2)/var(3) !"
End If
...
Dim ret As Long
On Error Resume Next  'audio-only media has no display window - "put ... window" would error out otherwise
If Multiplie <> 0 Then
'MiniAVI.Stinge_Aprinde_Lampi.Enabled = False
